...
class Table:
    ...
    def playHand(self, vocal=False):
        ...
        # move dealer chip
        self._dealer = 0
        while self._playing[self._dealer] != dealer: self._dealer = (
                                                                    self._dealer + 1) % self._s.numP
    ...
    def _dealHoleCards(self):
        ...
        for p in self._playing:
            ...
            if self._vocal: print p.getName() + '(' + str(
                p.getStack()) + ')', 'dealt', self._deck[0], 'and', self._deck[
                1]
    ...
    def _preFlop(self):
        ...
        self._s.actor = (
                        self._dealer + 3) % self._s.numP  # first player to act
        ...
        if self._vocal: print self._playing[
            sbPos].getName(), 'posts small blind of', self._smallBlind
        self._s.currBets[bbPos] += self._bigBlind
        self._playing[bbPos].removeChips(self._bigBlind)
        if self._vocal: print self._playing[
            bbPos].getName(), 'posts big blind of', self._bigBlind
    ...
    def _flip(self, numCards):
        ...
        if len(
            self._s.folded) + 1 == self._s.numP: return  # all players but one have folded
        ...
        self._s.actor = (
                        self._dealer + 1) % self._s.numP  # first actor is player after dealer
    ...
    def _payWinners(self):
        ...
        for p in self._playing:
            if not board:
                ...
                rank = self._eval.evaluate(board, [p.show()[0].toInt(),
                                                   p.show()[1].toInt()])
            ranks[p] = rank
    ...
    def _openBetting(self):
        ...
        while True:
            ...
            notAllinOrFold = []
            for i in range(self._s.numP):
                if i not in self._s.folded and i not in self._s.allIn: notAllinOrFold.append(
                    i)
            if len(
                notAllinOrFold) == 0: break  # break if all players are folded or all-in
            # break if last player's raise cannot be matched
            if len(notAllinOrFold) == 1 and self._s.currBets[
                notAllinOrFold[0]] == max(self._s.currBets): break
    ...
    def _parseAction(self, action):
        ...
        player = self._playing[actor]
        m = max(
            self._s.currBets)  # largest contribution that any player has in current pot
        currentBet = self._s.currBets[actor]

        if action[0] == 'check':
            if currentBet < m: raise Exception(
                'Player must call to remain in the pot.')
            ...
            if raiseTo < self._s.minRaise: raise Exception(
