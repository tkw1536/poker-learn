...
class SmartPlayer(Player):
    def _genGameFeatures(self, table, gameState):
        ...
        gameFeatures = (51 + len(table.getPlayers())) * [0]
        ...
        gameFeatures[42] = self._numRaises

        for i in range(8):
            gameFeatures[42 + i] = self._raiseValues[i]

        # player stack size
        for (i, p) in enumerate(table.getPlayers()):
            gameFeatures[50+i] = p._stack
